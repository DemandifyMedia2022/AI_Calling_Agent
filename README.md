...
Console features in `backend/agent.py`:
...
```bash
RUN_SINGLE_CALL=1 python -m backend.agent console
```
...
Campaigns are defined as a mapping in `backend/agent.py` (`CAMPAIGNS`) and consumed by the UI (`backend/app/main.py`). Available packs:
...
- `httpx` — Used by `backend/app/main.py` to fetch CDN assets (LiveKit Web SDK) with in-memory caching.
...
- Use `py -m uvicorn backend.app.main:app --host 0.0.0.0 --port 8000 --reload` if `python` isn’t on PATH.
...
- Console app exits immediately:
  - When `RUN_SINGLE_CALL=1`, it runs one session and exits by design. Run `python -m backend.agent` without that env for interactive mode.
